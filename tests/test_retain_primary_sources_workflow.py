...
def test_retention_is_manual_on_standard_runners_and_keeps_bytes_out_of_git() -> None:
    ...
    # The job never touches a declaration.
    assert "config/decision_cutoff.json" not in text.split("on:", 1)[1]
    assert "config/fundamentals_geography.json" not in text.split("on:", 1)[1]
