...
the committed custody record, the one finding that verification produced, and the re-record of
the custody record that followed as a separate decision.
...
## The record was then re-recorded, as a separate decision

The verification above was reported first and the committed record was left untouched, because
replacing a record is a decision that belongs in `DECISIONS.md` (`docs/custody/README.md`) rather
than a consequence of a job. That decision was subsequently taken.

The reason is the dated consequence of not taking it: **once the `32971677163` artifact expires on
2 September 2026, no obtainable copy can match the committed per-file digests, so `verify-custody`
would return exit code 2 on every future run against the replacement.** A record that cannot
verify against any obtainable artifact does not preserve evidence; it teaches readers to wave
through the exit code that exists to stop them, which is the habit this procedure exists to
prevent. Three responses were available:

1. Re-record against run `33483975614` with a decision entry. **Chosen.**
...
`docs/custody/greek-dam-official-history.json` now names `source_run_id` `33483975614`. It was
**regenerated by the tooling, not edited**: run
[`33489920268`](https://github.com/theIslander-oly/Greece-bess-Investment-stress-tester/actions/runs/33489920268)
produced it with `record_mode: record`, and the committed file is what that run emitted.
Against the superseded record the only changes are three provenance fields and the four per-file
digests case 4 predicts:

| Field | Superseded | Current |
|---|---|---|
| `source_run_id` | `32971677163` | `33483975614` |
| `published_artifact_digest_sha256` | `127915bc…4b198` | `de30f4cc…e48009` |
| `recorded_at_utc` | 2026-08-27T13:20:05Z | 2026-09-01T09:00:48Z |
| `archive_manifest.json` | `92d866f0…` | `fdbe48c2…` |
| `daily_manifest.json` | `ffa96b05…` | `de970e60…` |
| `henex_archived_prices.csv` | `9ac9fbe4…` | `2dc854a0…` |
| `henex_daily_prices.csv` | `1ebea398…` | `142d3950…` |

Every content fingerprint is identical, both `*.quality.json` digests are unchanged, and no file
changed size. The superseded record stays in Git history as the fingerprint of the originally
accepted artifact, so nothing is lost by the replacement.

Two supporting changes were needed and are worth naming. Re-recording previously required
**deleting** the committed record so that the "no record exists" branch would fire, which is
indistinguishable from tampering; a `record_mode` input makes it an explicit request instead, and
neither mode ever writes to `docs/custody/`. And the `Record official artifact custody` workflow's
`history_run_id` and `history_digest` defaults now point at run `33483975614`, so a dispatched or
push-triggered verification checks the artifact the record actually describes.
...
so its bytes are the accepted bytes. The `record` run above regenerated its record too, and that
regenerated record matches the committed one in all five per-file digests, so
`docs/custody/henex-entsoe-reconciliation.json` is unchanged.

Its own expiry, **3 September 2026 at 12:50 UTC**, is unaffected by this work, and it has no
replacement. A refresh was attempted on 1 September (run
[`33489364087`](https://github.com/theIslander-oly/Greece-bess-Investment-stress-tester/actions/runs/33489364087))
and was refused at the guard step: `ENTSOE_SECURITY_TOKEN` is no longer configured as a repository
secret, so no ENTSO-E request was made and no artifact was produced. Restoring that secret is an
operator action. Until then the encrypted upload of the existing artifact, which needs no secret,
is the only thing that preserves that evidence past 3 September.
